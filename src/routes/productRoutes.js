...
router.post('/add',
    ...
    async (req,res)=>{
        ...
        try{
            // check if the category is valid/exist
            ...
            return res.status(500).json({
                product:null, message:"unable to save product in DB"
            })
        }
})
/*
...
router.put('/updateStock/:id', async (req,res)=>{
    ...
    try{
        const product = await Product.findByIdAndDelete(id)
        res.status(200).json({
            product, message:"Updated Stock product in DB"
        })
        ...
        return res.status(500).json({
            product:null,
            message:"Unable to Update Stock in DB"
